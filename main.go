...
func challenge1_6() error {
    ...
    str1 := "this is a test"
    str2 := "wokka wokka!!!"
    hamDist, err := encoding.HammingDistanceByteSlice([]byte(str1), []byte(str2))
    if err != nil {
        fmt.Println(err)
    }
    ...
    var keySize int 
    var bestHammingDist float64 = math.MaxFloat64 

    for size := 2; size < 40; size++ {
        hamDist1, err := encoding.HammingDistanceByteSlice(decodedBytes[:size], decodedBytes[size:2*size])
        if err != nil {
            fmt.Println(err)
        }
        ...
        hammingDistNorm := float64(hamDist1) / float64(size)

        // Smallest normalized hamming distance replaces current best.
        if bestHammingDist > hammingDistNorm {
    ...
    blockSize := keySize
    blocks := make([][]byte, 0)
    for block := 0; block < len(decodedBytes) / keySize; block++ {
        // get block
        blocks = append(blocks, decodedBytes[block*blockSize:(block+1)*blockSize])
        // get key
        // xor block with key
        // get score
    ...
    blocksT := util.TransposeBytes(blocks)
